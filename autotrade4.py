...
MAX_ORDER_AGE = 60 # 오래된 주문 기준 시간 (초)
...
async def start_trading_strategy():
    global is_running, sell_price, sell_date, buying, count, waiting, order, leverage, max_pnl, AI_mode, infprofit
    ...
    while is_running:
        try:
            ...
            if AI_mode is False and buying is False and order is None:
                ...
                now = datetime.now()
                if sell_price != 0 and now.minute == 4: # 1시간마다 측정
...
async def get_status(ctx):
    global is_running, sell_price, sell_date, buying, count, waiting, order, leverage, max_pnl, n, infprofit, AI_mode
    current_price_info = client.get_symbol_ticker(symbol=f"{symbol}")
    current_price = float(current_price_info['price'])
    position_info = get_futures_position_info(symbol)
    unrealizedProfit = float(position_info['unRealizedProfit'])
