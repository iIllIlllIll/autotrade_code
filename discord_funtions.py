...
@bot.command(name='warningforloss')
...
!warningforloss : WARNINGFORLOSS 토글
...
async def start_trading_strategy():
    ...
    global msg_system, msg_system_warn
    global wait_up, latest_degree
...
'''
            ...
            msg_system = msg_stratagy + msg_system
...
# reason : {ready_reason}
''' + msg_current_status
            msg_system_warn = msg_stratagy + f"Now we have done with the {count} additional purchase" + msg_system_warn
            msg_user_warn = f'''
...
```
                    ...
                    elif WARNINGFORLOSS is True:  # warning 경고
...
```
                    ...
                    if analysis2_state is True:
                        if analysis2 != None:
                            if abs(float(analysis2.get('price'))-current_price)/current_price <= 0.002:    
                                percentage = 100 / (2 ** n)
                                order_price = round(current_price,1)
                                order = execute_limit_long_order(symbol, order_price, percentage, leverage)
                                iquantity = calculate_order_quantity(percentage)
            ...
            if buying is True and order is None:
                ...
                if difference_in_minutes >= 60*3 and float(latest_degree) >= 7:
                    if leverage*0.04*2 + 1 < pnl <= leverage*0.04*2 + 3:
                        order = close(symbol)
                        message(f"본전매도완료\n최고 PNL: {max_pnl}%\n현재 PNL: {pnl}%\nRealizedProfit : {unrealizedProfit}")
                        save_to_db(datetime.today(),pnl,unrealizedProfit, count, inv_amount, inv_amount+unrealizedProfit)
                        sell_price = current_price
