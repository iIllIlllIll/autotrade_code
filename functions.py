...
def get_5min_candles(symbol, interval='5m', lookback='1500 minutes ago UTC'):
...
def get_15min_candles(symbol, interval='15min', lookback='500 hours ago UTC'):
    klines = client.get_historical_klines(symbol, interval, lookback)
    return klines[-100:] 
    ...
    try:
        ...
        for position in positions:
            if float(position['positionAmt']) != 0:
                ...
                quantity = abs(float(position['positionAmt']))
                inv_amount = entryprice * abs(float(position['positionAmt'])) / leverage  # 투입금액
                pnl = unrealizedProfit / inv_amount

                quantitytoclose = usdt*leverage/entryprice
